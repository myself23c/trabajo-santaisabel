...
# Cargar el archivo con utf-8 y reemplazar errores
...
# Opcional: Guardar el archivo corregido para usos futuros
...
def guardar_cambios():
    """Guarda los cambios en la base de datos con codificación UTF-8."""
    data.to_csv(db_file, index=False, encoding='utf-8')
...
def menu():
    ...
    print("0. Salir")
    print("================================")
    print("Interface web es presionando ctrl + click en el link:  http://192.168.1.101:3000/")
...
def listar_expedientes():
    """Lista los números de expediente y sus integrantes en orden alfabético."""
    try:
        # Limpiar valores vacíos o no numéricos y convertir a enteros
        data['numero_de_expediente'] = (
            data['numero_de_expediente']
            .replace('', 0)  # Reemplaza valores vacíos con 0
            .fillna(0)  # Rellena valores NaN con 0
            .astype(float)  # Convierte a float para manejar decimales
            .astype(int)  # Convierte a entero
        )

        # Agrupar por número de expediente, ordenar nombres alfabéticamente
        expedientes = data.groupby('numero_de_expediente')['nombre_completo'].apply(
            lambda nombres: sorted(nombres)
        ).sort_index()
        ...
        print(f"Error al procesar los números de expediente: {e}")

def listar_expedientes_libres():
    """Lista los números de expediente libres del 1 al 300."""
    try:
        # Limpiar valores vacíos o no numéricos y convertir a enteros
        ocupados = (
            data['numero_de_expediente']
        ...
        # Encuentra números libres del 1 al 300
