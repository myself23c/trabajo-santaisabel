...
def main():
    ...
    print("=== Filtrar pacientes EMBARAZADAS por rango de fechas ===\n")
    ...
    with open(ruta, encoding="utf-8", newline="") as f:
        ...
        for fila in lector:
            ...
            if not (inicio <= fecha <= fin):
                continue
            # Filtrar sólo los casos de embarazada
            if fila.get("embarazada", "").strip().lower() != "embarazada":
                continue
            # Añadir los datos de interés
            pacientes.append({col: fila.get(col, "").strip() for col in columnas})

    if not pacientes:
        print("\nNo se encontraron pacientes embarazadas en ese rango de fechas.")
        return
    ...
    print(f"\nTotal de pacientes embarazadas: {len(pacientes)}")
