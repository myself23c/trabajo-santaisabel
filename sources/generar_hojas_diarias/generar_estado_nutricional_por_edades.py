...
def leer_nutricion():
    """
    Pide al usuario la categoría de nutrición y la valida.
    """
    opciones = ['desnutricion', 'bajo-peso', 'peso-normal', 'sobrepeso', 'obesidad']
    opts_str = ", ".join(opciones)
    while True:
        n = input(f"Categoría de nutrición ({opts_str}): ").strip().lower()
        if n in opciones:
            return n
        print("Opción inválida. Elige una de:", opts_str)

def main():
    ruta = "../../pacientes-diciembre-test-final.csv"
    print("=== Filtrar pacientes por fechas, edad y nutrición ===\n")
    ...
    with open(ruta, encoding="utf-8", newline="") as f:
        ...
        for fila in lector:
            ...
            # Nutrición
            if fila.get("nutricion", "").strip().lower() != nutricion_sel:
                continue
    ...
    # Cálculo de anchos para la tabla
    anchos = {}
    for col in columnas:
        max_len = max(len(col), *(len(p[col]) for p in pacientes))
        anchos[col] = max_len
