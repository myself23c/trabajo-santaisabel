...
def main():
    ...
    print("=== Filtrar pacientes REFERIDOS por rango de fechas ===\n")
    ...
    columnas = [
        ...
        "numero_de_expediente", "fecha_consulta", "diagnostico", "telefono"
    ]
    ...
    with open(ruta, encoding="utf-8", newline="") as f:
        ...
        for fila in lector:
            ...
            if not (inicio <= fecha <= fin):
                continue
            # Filtrar sólo los referidos
            if fila.get("referido", "").strip().lower() != "referido":
                continue
            # Añadir los datos de interés
            pacientes.append({col: fila.get(col, "").strip() for col in columnas})

    if not pacientes:
        print("\nNo se encontraron pacientes referidos en ese rango de fechas.")
        return
    ...
    print(f"\nTotal de pacientes referidos: {len(pacientes)}")
